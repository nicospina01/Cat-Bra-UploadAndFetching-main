const xlsx = require('xlsx');
const axios = require('axios');
const fs = require('fs');
const path = require('path');

// Configuración VTEX
const account = "tekno415"; // Reemplaza con tu cuenta VTEX
const authCookie = "eyJhbGciOiJFUzI1NiIsImtpZCI6IkM1OUU5NzkwNDhGQTQ3Q0I3QzVEMEQ2Q0VDQzVCRDU1RDNFQjVENjgiLCJ0eXAiOiJqd3QifQ..pMsmSHfXW1sN_pRkfCL-3g4-Be14TLXLbMbrgkHpl7voUnHJ8wGlGP4y6K_dw5uhLqmmXFy-YuxXuUaCpWrpOg"; // Reemplaza con tu VtexAuthcookie

// Función para leer la plantilla y realizar las solicitudes POST
async function uploadBrands() {
  const filePath = path.join(__dirname, 'templates/template-brands.xlsx');
  const workbook = xlsx.readFile(filePath);
  const sheetName = workbook.SheetNames[0];
  const worksheet = workbook.Sheets[sheetName];
  const jsonData = xlsx.utils.sheet_to_json(worksheet, { defval: '' });

  for (const row of jsonData) {
    const brandPayload = {
      name: row.Name.toString(),
      isActive: row.IsActive.toString().toLowerCase() === 'true'
    };

    try {
      const brandCreationUrl = `https://${account}.vtexcommercestable.com.br/api/catalog-seller-portal/brands`;
      const response = await axios.post(brandCreationUrl, brandPayload, {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
          'VtexIdclientAutCookie': authCookie
        }
      });
      console.log(`Marca creada: ${brandPayload.name}, ID: ${response.data.Id}`);
    } catch (apiError) {
      console.error(`Error creando marca ${brandPayload.name}:`, apiError.message);
    }
  }
}

// Ejecutar la función
uploadBrands();